...
## v1.5.0 beta.12 persistent FPS row pass

- The FPS row now follows the user's metric visibility setting instead of disappearing when a target temporarily stops presenting during an application switch.
- `NoFrames`, startup and transient unavailable samples render as `FPS --` without exposing diagnostic text or collapsing the HUD layout; the next valid active sample restores the numeric value automatically.
- Overlay, controller and target-window tracking checks: 43 passed. Full automated result: 322 passed, 0 failed, 0 skipped.
- Portable single-file size: 8,990,208 bytes. SHA-256: `4C0DE36BF070EC64E762F245E45FFE4DD059A3FD8F4453CDC4478B5345ED4C28`.

## v1.5.0 beta.11 two-column storage pass

- The detail panel now lays out fixed-drive cards two per row at the standard 460-DIP window width, including the width reserved by an active vertical scroll bar.
- Each drive card was reduced from 220 x 128 DIP to 186 x 96 DIP while retaining the drive label, used/total capacity, usage percentage, tooltip and progress bar.
- Full automated result: 322 passed, 0 failed, 0 skipped. Portable single-file size: 8,990,208 bytes. SHA-256: `242074D3568D65330280BE32F91DF00FABCD6967E6C754B144AE92D1E0B35189`.
