...
- `Present FPS` 来自 PresentMon 2.5.1 对 Windows ETW Present 事件的只读观测；它不是游戏引擎内部 FPS，也不是 Displayed FPS。
- CPU、内存和 GPU 百分比均标注为系统总体；温度和当前频率只在数据源真实提供时显示。
- 默认游戏安全模式不构造 LibreHardwareMonitor、CPU 提权助手或相关硬件访问驱动。兼容传感器是需要重启的可选模式，且该运行不启用游戏悬浮窗。
