...
public sealed class GameOverlayController : IAsyncDisposable
{
    ...
    internal GameOverlayController(
        IGameOverlayFrameProvider frameProvider,
        IMonitorService monitorService,
        ForegroundTargetTracker targetTracker,
        IGameOverlayView view,
        Action<Action>? uiDispatcher)
    {
        ...
        _uiDispatcher = uiDispatcher ?? DefaultRunOnUi;
    ...
    private void RunOnUi(Action action) => _uiDispatcher(action);

    private static void DefaultRunOnUi(Action action)
    {
