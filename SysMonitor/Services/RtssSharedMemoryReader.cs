...
internal static class RtssSharedMemoryParser
{
    ...
    internal static SharedMemoryValue Parse(ReadOnlySpan<byte> data, int processId, uint tickCount)
    {
        ...
        for (uint index = 0; index < entryCount; index++)
        {
            ...
            // 3. Check frame counter with elapsed time
...
{
    internal static readonly string[] MappingNames = ["RTSSSharedMemoryV2", "RTSSSharedMemory"];
    ...
    public SharedMemoryValue Read(int processId)
    {
        foreach (string mappingName in MappingNames)
        {
            try
            {
                ...
                for (int attempt = 0; attempt < 3; attempt++)
                {
                    ...
                    if (result is SharedMemoryValue value && value.HasValue)
                    {
                        return value;
                    }
                }
        ...
        return SharedMemoryValue.Missing("RTSS mapping unavailable or target not hooked");
    }
