...
public sealed class RtssLegacyCompatibilityService
{
    ...
    /// <summary>Finds the real RTSS installation across Registry, running processes, and fixed drives.</summary>
    public static bool TryFindRtssInstallation(out string? rtssExePath, out string? rtssRootDirectory)
    {
        rtssExePath = null;
        rtssRootDirectory = null;
        try
        {
            // 1. Check registry (both 32-bit and 64-bit views)
            string[] registryKeys = new[]
            {
                @"SOFTWARE\WOW6432Node\Unwinder\RTSS",
                @"SOFTWARE\Unwinder\RTSS",
                @"SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\RTSS.exe"
            };
            ...
            foreach (var keyName in registryKeys)
            {
                try
                {
                    ...
                    {
                        string exe = installPath.EndsWith(".exe", StringComparison.OrdinalIgnoreCase) ? installPath : Path.Combine(installPath, "RTSS.exe");
                        if (File.Exists(exe))
                        {
                            rtssExePath = exe;
                            rtssRootDirectory = Path.GetDirectoryName(exe);
                            return true;
                        }
                    }
                }
            ...
            // 2. Check running RTSS processes
            foreach (var process in Process.GetProcessesByName("RTSS"))
            {
                try
                {
                    ...
                    {
                        rtssExePath = image;
                        rtssRootDirectory = Path.GetDirectoryName(image);
                        return true;
                    }
                }
                catch { }
            ...
            // 3. Check well-known directories across all available fixed drives
            foreach (var drive in DriveInfo.GetDrives())
            {
                try
                {
                    ...
                    {
                        Path.Combine(driveRoot, "MSI After", "RivaTuner Statistics Server", "RTSS.exe"),
                        Path.Combine(driveRoot, "MSI Afterburner", "RivaTuner Statistics Server", "RTSS.exe"),
                        Path.Combine(driveRoot, "Program Files (x86)", "RivaTuner Statistics Server", "RTSS.exe"),
                        Path.Combine(driveRoot, "Program Files", "RivaTuner Statistics Server", "RTSS.exe"),
                        Path.Combine(driveRoot, "RivaTuner Statistics Server", "RTSS.exe"),
                        Path.Combine(driveRoot, "RTSS", "RTSS.exe")
                    };
                    ...
                    foreach (var cand in candidates)
                    {
                        ...
                        {
                            rtssExePath = cand;
                            rtssRootDirectory = Path.GetDirectoryName(cand);
                            return true;
                        }
                    }
                }
    ...
    {
        string? root = null;
        if (TryFindRtssInstallation(out _, out string? detectedRoot))
        {
            root = detectedRoot;
        }
    ...
    public bool TryAutoEnableForExecutable(string executablePath, out string? diagnostic)
    {
        diagnostic = null;
        try
        {
            RtssCompatibilityResult status = GetStatus(executablePath);
            if (status.Enabled)
            {
                // Profile is ready — ensure RTSS is running so the hook takes effect.
                _ = TryEnsureRtssRunning(out _);
                return true;
            }
            ...
            RtssCompatibilityResult result = Enable(executablePath);
            diagnostic = result.Diagnostic;
            if (result.Success)
            {
                // Auto-start RTSS in the background so the user never needs to
                // launch it manually. This is the key to zero-config old-game support.
                _ = TryEnsureRtssRunning(out _);
            }
    ...
    public static bool TryEnsureRtssRunning(out string? diagnostic)
    {
        diagnostic = null;
        if (IsRtssRunning())
        {
        ...
        try
        {
            if (TryFindRtssInstallation(out string? exePath, out _) && !string.IsNullOrWhiteSpace(exePath) && File.Exists(exePath))
            {
            ...
            diagnostic = "RTSS executable not found.";
            return false;
        }
        catch (Exception ex)
        {
    ...
    private static byte[] PatchProfile(byte[] bytes)
    {
        ...
        if (sections.Count == 0)
        {
            ...
            lines.Add(new LineSegment("[Hooking]", nl)); lines.Add(new LineSegment("EnableHooking=1", nl)); lines.Add(new LineSegment("HookDirectDraw=1", nl)); lines.Add(new LineSegment("HookD3D8=1", string.Empty));
        ...
        foreach (var key in new[] { "EnableHooking", "HookDirectDraw", "HookD3D8" }) keyIndexes[key] = new List<int>();
        for (int i = start + 1; i < end; i++)
        {
            var match = System.Text.RegularExpressions.Regex.Match(lines[i].Content, @"^\s*(EnableHooking|HookDirectDraw|HookD3D8)\s*=", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        ...
        foreach (var kv in keyIndexes)
            ...
            {
                if (end > 0 && end <= lines.Count && lines[end - 1].Ending.Length == 0)
                {
                    lines[end - 1] = lines[end - 1] with { Ending = nlExisting };
                }
                lines.Insert(end++, new LineSegment(kv.Key + "=1", nlExisting));
            }
        return Encoding.UTF8.GetBytes(JoinLines(lines));
    }
