...
public sealed class RtssLegacyCompatibilityServiceTests : IDisposable
{
    ...
    [Fact]
    public void TryAutoEnableForExecutableSuccessfullyAppliesProfile()
    {
        Assert.True(_service.TryAutoEnableForExecutable(_exe, out string? diagnostic));
        Assert.Equal("Compatibility enabled.", diagnostic);
        var profile = Path.Combine(_profiles, "Game.exe.cfg");
        Assert.True(File.Exists(profile));
        Assert.Equal("[Hooking]\r\nEnableHooking=1\r\nHookDirectDraw=1\r\nHookD3D8=1\r\n", File.ReadAllText(profile));
        // Calling it again when already enabled returns true cleanly
        Assert.True(_service.TryAutoEnableForExecutable(_exe, out diagnostic));
    }
